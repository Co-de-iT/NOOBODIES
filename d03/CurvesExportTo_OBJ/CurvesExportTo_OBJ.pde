/*
 ...
 exports Vec3D ArrayLists to OBJ curves
...
void draw() {
  ...
  for (int i=0; i<curves.size (); i++) {
    ...
    for (int j=0; j< pts.size (); j++) {
      Vec3D vc = pts.get(j);
      stroke(color(lerpColor(color(255, 0, 0), color(255), float(j)/pts.size())));
...
void keyPressed() {
  ...
  if (key=='e') exportCurvesAsObj(curves, "curves", new Vec3D(), new Vec3D(1, 1, 1));
}
